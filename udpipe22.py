...
class UDPipe2:
    ...
    def __init__(self, threads, seed=42):
        ...
        graph = tf.Graph()
        graph.seed = seed
        self.session = tf.compat.v1.Session(graph = graph, config=tf.compat.v1.ConfigProto(inter_op_parallelism_threads=threads,
    ...
    def construct(self, args, train, devs, tests, predict_only):
        ...
        with self.session.graph.as_default():
            # Inputs
            self.sentence_lens = tf.compat.v1.placeholder(tf.int32, [None])
            self.word_ids = tf.compat.v1.placeholder(tf.int32, [None, None])
            self.charseqs = tf.compat.v1.placeholder(tf.int32, [None, None])
            self.charseq_lens = tf.compat.v1.placeholder(tf.int32, [None])
            self.charseq_ids = tf.compat.v1.placeholder(tf.int32, [None, None])
            if train.variants > 1: self.variants = tf.compat.v1.placeholder(tf.int32, [None])
            if train.embeddings_size: self.embeddings = tf.compat.v1.placeholder(tf.float32, [None, None, train.embeddings_size])
            self.tags = dict((tag, tf.compat.v1.placeholder(tf.int32, [None, None])) for tag in args.tags)
            self.heads = tf.compat.v1.placeholder(tf.int32, [None, None])
            self.deprels = tf.compat.v1.placeholder(tf.int32, [None, None])
            ...
            inputs = []
            if args.we_dim:
                word_embeddings = tf.compat.v1.get_variable("word_embeddings", shape=[num_words, args.we_dim], dtype=tf.float32)
                inputs.append(tf.nn.embedding_lookup(params=word_embeddings, ids=self.word_ids))

            # Character-level embeddings
            character_embeddings = tf.compat.v1.get_variable("character_embeddings", shape=[num_chars, args.cle_dim], dtype=tf.float32)
            characters_embedded = tf.nn.embedding_lookup(params=character_embeddings, ids=self.charseqs)
            characters_embedded = tf.compat.v1.layers.dropout(characters_embedded, rate=args.dropout, training=self.is_training)
            _, (state_fwd, state_bwd) = tf.compat.v1.nn.bidirectional_dynamic_rnn(
                tf.compat.v1.nn.rnn_cell.GRUCell(args.cle_dim), tf.compat.v1.nn.rnn_cell.GRUCell(args.cle_dim),
            ...
            # Variants
            if train.variants > 1:
                variant_embeddings = tf.compat.v1.get_variable("variant_embeddings", shape=[train.variants, args.variant_dim], dtype=tf.float32)
                variant_embeddings = tf.nn.embedding_lookup(params=variant_embeddings, ids=self.variants)
                variant_embeddings = tf.expand_dims(variant_embeddings, axis=1)
                variant_embeddings = tf.tile(variant_embeddings, [1, tf.shape(input=self.word_ids)[1], 1])
            ...
            # Shared RNN layers
            hidden_layer = tf.compat.v1.layers.dropout(inputs, rate=args.dropout, training=self.is_training)
            for i in range(args.rnn_layers):
                (hidden_layer_fwd, hidden_layer_bwd), _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(
                ...
                hidden_layer = tf.compat.v1.layers.dropout(hidden_layer_fwd + hidden_layer_bwd, rate=args.dropout, training=self.is_training)
            ...
            for i in range(args.rnn_layers_tagger):
                ...
                tag_hidden_layer = tf.compat.v1.layers.dropout(hidden_layer_fwd + hidden_layer_bwd, rate=args.dropout, training=self.is_training)
                if i: tag_hidden_layer += previous
            for tag in args.tags:
                tag_layer = tag_hidden_layer
                for _ in range(args.tag_layers):
                    tag_layer += tf.compat.v1.layers.dropout(tf.compat.v1.layers.dense(tag_layer, args.rnn_cell_dim, activation=tf.nn.tanh), rate=args.dropout, training=self.is_training)
                if tag == "LEMMAS": tag_layer = tf.concat([tag_layer, cle_inputs[:, 1:]], axis=2)
                output_layer = tf.compat.v1.layers.dense(tag_layer, num_tags[tag])
                ...
                if args.label_smoothing:
                    ...
                    loss += tf.compat.v1.losses.softmax_cross_entropy(gold_labels, output_layer, weights=weights)
                else:
                    loss += tf.compat.v1.losses.sparse_softmax_cross_entropy(self.tags[tag], output_layer, weights=weights)
            ...
            if args.parse:
                ...
                if args.rnn_layers == 0:
                    ...
                    for tag in ["UPOS", "XPOS", "FEATS"]:
                        parser_inputs.append(tf.nn.embedding_lookup(params=tf.compat.v1.get_variable(tag + "_embeddings", shape=[num_tags[tag], 128], dtype=tf.float32),
                                                                    ids=tf.pad(tensor=self.predictions[tag], paddings=((0, 0),(1, 0)), constant_values=2)))
                    parser_inputs = tf.concat(parser_inputs, axis=2)
                    hidden_layer = tf.compat.v1.layers.dropout(parser_inputs, rate=args.dropout, training=self.is_training)
                ...
                for _ in range(args.parser_layers):
                    head_deps += tf.compat.v1.layers.dropout(tf.compat.v1.layers.dense(head_deps, args.rnn_cell_dim, activation=tf.nn.tanh), rate=args.dropout, training=self.is_training)
                head_roots = hidden_layer
                for _ in range(args.parser_layers):
                    head_roots += tf.compat.v1.layers.dropout(tf.compat.v1.layers.dense(head_roots, args.rnn_cell_dim, activation=tf.nn.tanh), rate=args.dropout, training=self.is_training)

                head_deps_bias = tf.compat.v1.get_variable("head_deps_bias", [args.rnn_cell_dim], dtype=tf.float32, initializer=tf.compat.v1.zeros_initializer)
                head_roots_bias = tf.compat.v1.get_variable("head_roots_bias", [args.rnn_cell_dim], dtype=tf.float32, initializer=tf.compat.v1.zeros_initializer)
                head_biaffine = tf.compat.v1.get_variable("head_biaffine", [args.rnn_cell_dim, args.rnn_cell_dim], dtype=tf.float32, initializer=tf.compat.v1.zeros_initializer)
                ...
                if args.label_smoothing:
                    ...
                    gold_labels += args.label_smoothing / tf.cast(max_words + 1, dtype=tf.float32)
                    loss += tf.compat.v1.losses.softmax_cross_entropy(gold_labels, heads, weights=weights)
                else:
                    loss += tf.compat.v1.losses.sparse_softmax_cross_entropy(self.heads, heads, weights=weights)

                # Deprels
                self.deprel_hidden_layer = tf.identity(hidden_layer)
                self.deprel_heads = tf.identity(self.heads)

                deprel_deps = tf.compat.v1.layers.dropout(tf.compat.v1.layers.dense(self.deprel_hidden_layer[:, 1:], args.parser_deprel_dim, activation=tf.nn.tanh), rate=args.dropout, training=self.is_training)
                for _ in range(args.parser_layers - 1):
                    deprel_deps += tf.compat.v1.layers.dropout(tf.compat.v1.layers.dense(deprel_deps, args.parser_deprel_dim, activation=tf.nn.tanh), rate=args.dropout, training=self.is_training)

                deprel_indices = tf.stack([
                    tf.tile(tf.expand_dims(tf.range(tf.shape(input=self.deprel_heads)[0]), axis=1), multiples=[1, tf.shape(input=self.deprel_heads)[1]]),
                    self.deprel_heads], axis=2)
                deprel_roots = tf.gather_nd(self.deprel_hidden_layer, deprel_indices, )
                deprel_roots = tf.compat.v1.layers.dropout(tf.compat.v1.layers.dense(deprel_roots, args.parser_deprel_dim, activation=tf.nn.tanh), rate=args.dropout, training=self.is_training)
                for _ in range(args.parser_layers - 1):
                    deprel_roots += tf.compat.v1.layers.dropout(tf.compat.v1.layers.dense(deprel_roots, args.parser_deprel_dim, activation=tf.nn.tanh), rate=args.dropout, training=self.is_training)

                deprel_deps_bias = tf.compat.v1.get_variable("deprel_deps_bias", [args.parser_deprel_dim], dtype=tf.float32, initializer=tf.compat.v1.zeros_initializer)
                deprel_roots_bias = tf.compat.v1.get_variable("deprel_roots_bias", [args.parser_deprel_dim], dtype=tf.float32, initializer=tf.compat.v1.zeros_initializer)
                deprel_biaffine = tf.compat.v1.get_variable("deprel_biaffine", [args.parser_deprel_dim, num_deprels * args.parser_deprel_dim], dtype=tf.float32, initializer=tf.compat.v1.zeros_initializer)
                ...
                if args.label_smoothing:
                    ...
                    loss += tf.compat.v1.losses.softmax_cross_entropy(gold_labels, deprels, weights=weights)
                else:
                    loss += tf.compat.v1.losses.sparse_softmax_cross_entropy(self.deprels, deprels, weights=weights)
            ...
            # Summaries
            summary_writer = tf.compat.v2.summary.create_file_writer(logdir=args.model, flush_millis=10 * 1000)
            with summary_writer.as_default(), tf.contrib.summary.record_summaries_every_n_global_steps(100):
                self.training_summaries = [
                    tf.compat.v2.summary.scalar(name="train/loss", data=loss, step=tf.compat.v1.train.get_or_create_global_step()),
                    tf.compat.v2.summary.scalar(name="train/lr", data=self.learning_rate, step=tf.compat.v1.train.get_or_create_global_step())]
                if args.clip_gradient:
                    self.training_summaries.append(tf.compat.v2.summary.scalar(name="train/gradient_norm", data=gradient_norm, step=tf.compat.v1.train.get_or_create_global_step()))
                for tag in args.tags:
                    self.training_summaries.append(tf.compat.v2.summary.scalar(
                        ...
                        weights_sum, step=tf.compat.v1.train.get_or_create_global_step()))
                if args.parse:
                    ...
                    self.training_summaries.extend([tf.compat.v2.summary.scalar(name="train/heads_acc", data=heads_acc, step=tf.compat.v1.train.get_or_create_global_step())])
                    deprels_acc = tf.reduce_sum(input_tensor=tf.cast(tf.equal(self.deprels, tf.argmax(input=deprels, axis=-1, output_type=tf.int32)),
                                                        tf.float32) * weights) / weights_sum
                    self.training_summaries.extend([tf.compat.v2.summary.scalar(name="train/deprels_acc", data=deprels_acc, step=tf.compat.v1.train.get_or_create_global_step())])

            # Evaluation summaries
            self.summary_writers = {"": summary_writer}
            for dataset in devs + tests:
                if dataset.label not in self.summary_writers:
                    self.summary_writers[dataset.label] = tf.compat.v2.summary.create_file_writer(
                        logdir=os.path.join(args.model, dataset.label), flush_millis=10 * 1000)

            self.event = tf.compat.v1.placeholder(tf.string, [])
            ...
            for summary_writer in self.summary_writers.values():
                with summary_writer.as_default():
                    tf.compat.v1.summary.initialize(session=self.session)
    ...
    def load(self, path):
        ...
        if os.path.exists(consistent_feats_table):
            ...
            with self.session.graph.as_default():
                consistent_feats_table = tf.convert_to_tensor(value=consistent_feats_table, dtype=tf.float32)
                self.predictions["FEATS"] = tf.argmax(
                    input=tf.nn.softmax(self.predictions_logits["FEATS"], axis=2) * tf.gather(consistent_feats_table, self.predictions["UPOS"]),
    ...
    def evaluate(self, dataset_name, dataset, args):
        ...
        event = [tf.compat.v1.Summary.Value(tag="{}/{}".format(dataset_name, metric), simple_value=metrics[metric].f1) for metric in self.METRICS]
        event = tf.compat.v1.Event(summary=tf.compat.v1.Summary(value=event), step=self.session.run(self.global_step), wall_time=time.time()).SerializeToString()
